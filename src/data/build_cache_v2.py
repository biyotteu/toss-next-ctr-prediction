...
def analyze_schema_and_stats(parquet_path: str,
                             ...
                             group_key: str,
                             impute_strategy: str,
                             num_cols_explicit: List[str] | None = None) -> Dict:
...
def build_sharded_cache(
    ...
    *,
    ...
    # --- NEW: co-vis join 옵션 ---
    covis_enabled: bool = False,
    covis_dir: str = "./cache/covis"
) -> str:
    """
    ...
    schema = analyze_schema_and_stats(
        parquet_path, target_col, seq_col, cat_cols, num_patterns, group_key, impute_strategy, num_cols_explicit
    )
    num_cols = schema["num_cols"]; med_map = schema["med_map"]; total_n = schema["n_rows"]

    # --- NEW: co-vis 컬럼 준비(스키마에 미리 넣어둬야 _process_arrow_batch가 선택 가능) ---
    covis_cols = []
    covis_join_df = None
    if covis_enabled:
        if is_train:
            covis_path = os.path.join(covis_dir, "rowfeat_oof_all.parquet")
            covis_join_df = pl.read_parquet(covis_path)  # rid, 여러 co-vis 수치 컬럼
            join_key = "rid"
        else:
            covis_path = os.path.join(covis_dir, "rowfeat_test.parquet")
            covis_join_df = pl.read_parquet(covis_path)  # ID, 여러 co-vis 수치 컬럼
            join_key = "ID"
        # 조인 키 외의 수치 컬럼명 추출
        covis_cols = [c for c,dt in zip(covis_join_df.columns, covis_join_df.dtypes)
                        if c not in ("rid","ID") and (dt in (pl.Float32, pl.Float64, pl.Int32, pl.Int64, pl.UInt32, pl.UInt64))]
        # num_cols에 합치고, med_map 기본값(0.0) 추가
        for c in covis_cols:
    ...
    for rb in it:
        df = pl.from_arrow(rb)
        # --- NEW: 전역 rid 부여(train에서 co-vis 조인 키로 사용)
        if is_train:
            df = df.with_columns(pl.arange(row_cursor, row_cursor + df.height).alias("rid"))
        row_cursor += df.height

        # --- NEW: co-vis 조인 ---
        if covis_enabled:
            if is_train:
                # rid로 left join
                df = df.join(covis_join_df, on="rid", how="left")
                # 조인 후 rid는 더 이상 필요 없으면 생략 가능
                # df = df.drop("rid")
            else:
                if "ID" in df.columns:
                    df = df.join(covis_join_df, on="ID", how="left")
            # co-vis 결측 → 0.0
            if covis_cols:
                df = df.with_columns([pl.col(c).fill_null(0.0) for c in covis_cols])
